...
// This procedure is just a wrapper for the ugly cblas_dgemm call
...
int main(int argc, char** argv) {
  ...
  // Check if a matrix size and number of threads are given on the command line
  if (argc>1) {
    size = atoi(argv[1]);
    printf("Size is %d \n", size);
  }
  ...
  openblas_set_num_threads(nthreads);   // Set the number of threads
